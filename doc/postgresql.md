...
**另：PostgreSQL默认只监听本地端口，用`netstat -tuln`只会看到`tcp 127.0.0.1:5432 LISTEN`。修改postgresql.conf中的`listen_address=*`，监听所有端口，这样远程才能通过TCP/IP登录数据库，用`netstat -tuln`会看到`tcp 0.0.0.0:5432 LISTEN`。
